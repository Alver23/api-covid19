...
export class CovidService {
  ...
  public getCases(): Observable<CovidResponse> {
    return new Observable((observer: Observer<any>) => {
      combineLatest(this.getTotalCases(), this.getCasesByType(FIELD_TYPES.CITY), this.getCasesByType(FIELD_TYPES.STATE)).subscribe(
  ...
  public getCasesRecovered(): Observable<CovidResponse> {
    return new Observable((observer: Observer<any>) => {
      combineLatest(this.getTotalCasesByAttention(`'${ATTENTION_TYPES.RECOVERED}'`), this.getCasesByAttention(FIELD_TYPES.CITY, ATTENTION_TYPES.RECOVERED), this.getCasesByAttention(FIELD_TYPES.STATE, ATTENTION_TYPES.RECOVERED)).subscribe(
        ([totalCases, cities, state]) => {
          observer.next({
            ...new CCovidResponse({ totalCases, cities, state }),
          });
  ...
  public getCasesDeaths(): Observable<CovidResponse> {
    return new Observable((observer: Observer<any>) => {
      combineLatest(this.getTotalCasesByAttention(`'${ATTENTION_TYPES.DEATHS}'`), this.getCasesByAttention(FIELD_TYPES.CITY, ATTENTION_TYPES.DEATHS), this.getCasesByAttention(FIELD_TYPES.STATE, ATTENTION_TYPES.DEATHS)).subscribe(
        ([totalCases, cities, state]) => {
          observer.next({
            ...new CCovidResponse({ totalCases, cities, state }),
          });
